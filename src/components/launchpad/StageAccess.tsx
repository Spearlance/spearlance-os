...
export function StageAccess({ submissionId, initialData, onContinue, onBack, onSaveExit }: StageAccessProps) {
  ...
  const form = useForm<AccessData>({
    ...
    defaultValues: initialData || {
      ...
      storage: {},
      reporting: {},
      consent: { ack_shared_access: false },
    },
  });
      ...
      if (selectedClient) {
        ...
        if (Object.keys(updates).length > 0) {
          const { data: clientData } = await supabase
            .from("clients")
            .select("drive_folder_url, canva_folder_url, oviond_url")
        ...
        <div className="space-y-8 bg-card p-6 rounded-lg border">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                {form.formState.errors.web?.platform && (
                  <p className="text-xs text-destructive mt-1">{form.formState.errors.web.platform.message}</p>
                )}
              </div>
              <div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input id="oviond_url" placeholder="https://app.oviond.com/..." {...form.register("reporting.oviond_url")} />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start space-x-2">
              <Checkbox
                id="ack_shared_access"
                checked={form.watch("consent.ack_shared_access")}
                onCheckedChange={(checked) => form.setValue("consent.ack_shared_access", checked as boolean)}
              />
              <div className="space-y-1">
                ...
                {form.formState.errors.consent?.ack_shared_access && (
                  <p className="text-xs text-destructive">{form.formState.errors.consent.ack_shared_access.message}</p>
                )}
              </div>
            </div>
          </div>
