-- Fix notify_ticket_message function to use correct column name for message body
...
BEGIN
  ...
  IF NEW.author_user_id != v_ticket.requester_user_id AND NOT NEW.is_internal_note THEN
    ...
    VALUES (
      ...
      v_ticket.client_id,
      jsonb_build_object('ticket_id', NEW.ticket_id, 'message_preview', LEFT(NEW.body_richtext, 100))
    );
