...
Deno.serve(async (req) => {
  ...
  const daily: { date: string; source: string; leads: number }[] = [];
  ...
  for (const row of funnelRows) {
    ...
    daily.push({ date: row.lead_date, source: row.source, leads: row.total_count });
  }
  daily.sort((a, b) => a.date.localeCompare(b.date) || a.source.localeCompare(b.source));
