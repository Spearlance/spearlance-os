/**
 * SpearlanceOS Analytics Tracker (SOS) - DEBUG VERSION
 * Version 1.0.0-debug
 * Lightweight, privacy-first web analytics with verbose console logging
 */
(function() {
  'use strict';
  
  console.log('[SOS] Initializing debug version...');
  
  // Namespace safety - exit if already loaded
  if (window.sos) {
    console.warn('[SOS] Already loaded, exiting');
    return;
  }
  ...
  function getStorage(key) {
    try {
      ...
      console.warn('[SOS] localStorage.getItem failed:', e);
      return null;
    }
  }
  ...
  function setStorage(key, value) {
    try {
      ...
      console.warn('[SOS] localStorage.setItem failed:', e);
    }
  }
  ...
  function send(eventData) {
    console.log('[SOS] send() called with:', eventData);
    
    if (!config.collectorUrl || !config.workspaceKey) {
      console.error('[SOS] Cannot send - missing config:', {
        hasCollectorUrl: !!config.collectorUrl,
        hasWorkspaceKey: !!config.workspaceKey,
        collectorUrl: config.collectorUrl,
        workspaceKey: config.workspaceKey ? config.workspaceKey.substring(0, 15) + '...' : null
      });
      return;
    }
    
    if (!consentGranted || isOptedOut()) {
      console.warn('[SOS] Cannot send - no consent or opted out:', {
        consentGranted,
    ...
    console.log('[SOS] Sending payload to:', config.collectorUrl);
    ...
    try {
      if (navigator.sendBeacon) {
        console.log('[SOS] Using navigator.sendBeacon');
        const blob = new Blob([data], { type: 'application/json' });
        const result = navigator.sendBeacon(config.collectorUrl, blob);
        console.log('[SOS] sendBeacon result:', result);
      } else {
        console.log('[SOS] Using fetch (sendBeacon not available)');
        fetch(config.collectorUrl, {
          ...
          console.log('[SOS] Fetch response:', response.status, response.statusText);
          return response.text();
        }).then(function(text) {
          console.log('[SOS] Fetch response body:', text);
        }).catch(function(error) {
          console.error('[SOS] Fetch error:', error);
        });
      }
    } catch (e) {
      console.error('[SOS] send() exception:', e);
    }
  }
  
  // Track page view
  function trackPageView() {
    console.log('[SOS] trackPageView() called');
    try {
      ...
      if (isEntry) {
        ...
        try {
          ...
          console.warn('[SOS] sessionStorage.setItem failed:', e);
        }
      }
      ...
      console.log('[SOS] Page view data:', {
    ...
    try {
      ...
      [25, 50, 75, 100].forEach(function(threshold) {
        if (scrollPercentage >= threshold && !scrollTracked[threshold]) {
          ...
          console.log('[SOS] Scroll depth reached:', threshold + '%');
          send({
            type: 'scroll_depth',
            path: window.location.pathname,
            value: threshold
      ...
      console.error('[SOS] trackScrollDepth() error:', e);
    }
  }
  ...
  function trackEngagedTime() {
    try {
      ...
      if (engagedTotal >= 15000) {
        console.log('[SOS] Engaged time threshold reached:', Math.round(engagedTotal / 1000) + 's');
        send({
          type: 'engaged_time',
          path: window.location.pathname,
          value: Math.round(engagedTotal / 1000)
        });
        engagedTotal = 0;
      }
    } catch (e) {
      console.error('[SOS] trackEngagedTime() error:', e);
    }
  }
  ...
  window.sos = {
    ...
    config: config, // Expose config for debugging
    
    // Initialize with config
    init: function(opts) {
      console.log('[SOS] init() called with:', opts);
      try {
        ...
        console.log('[SOS] Config set:', {
          collectorUrl: config.collectorUrl,
          workspaceKey: config.workspaceKey ? config.workspaceKey.substring(0, 15) + '...' : null,
          enablePopupConsent: config.enablePopupConsent
        });
        
        // Initialize session
        sessionId = getStorage('sos_sid') || generateId();
        setStorage('sos_sid', sessionId);
        console.log('[SOS] Session ID:', sessionId);
        
        userId = getStorage('sos_uid');
        console.log('[SOS] User ID:', userId || 'none');
        
        // Check existing consent
        const storedConsent = getStorage('sos_consent');
        console.log('[SOS] Stored consent:', storedConsent);
        ...
        try {
          ...
          console.warn('[SOS] sessionStorage.getItem failed:', e);
        }
        
        // Show popup if needed
        if (config.enablePopupConsent && !storedConsent && !isOptedOut()) {
          console.log('[SOS] Showing consent popup');
          window.sos._showConsentPopup();
        } else if (consentGranted) {
          console.log('[SOS] Consent already granted, starting tracking');
          window.sos._startTracking();
        }
      } catch (e) {
        console.error('[SOS] init() error:', e);
      }
    },
    
    // Set consent
    consent: function(state) {
      console.log('[SOS] consent() called with:', state);
      try {
        if (state === 'granted') {
          consentGranted = true;
          setStorage('sos_consent', 'granted');
          console.log('[SOS] Consent granted, starting tracking');
          window.sos._startTracking();
        } else if (state === 'denied') {
          consentGranted = false;
          setStorage('sos_consent', 'denied');
          console.log('[SOS] Consent denied');
        }
      } catch (e) {
        console.error('[SOS] consent() error:', e);
      }
    },
    
    // Identify user
    identify: function(id) {
      console.log('[SOS] identify() called with:', id);
      try {
        userId = id;
        setStorage('sos_uid', id);
      } catch (e) {
        console.error('[SOS] identify() error:', e);
      }
    },
    
    // Manual page track
    page: function() {
      console.log('[SOS] page() called manually');
      trackPageView();
    },
    
    // Track custom event
    track: function(evt) {
      console.log('[SOS] track() called with:', evt);
      try {
        send(evt);
      } catch (e) {
        console.error('[SOS] track() error:', e);
      }
    },
    
    // Internal: Start tracking
    _startTracking: function() {
      console.log('[SOS] _startTracking() called');
      
      // Initial page view
      if (config.autoTrackPageViews) {
        console.log('[SOS] Auto-tracking page views enabled');
        trackPageView();
      }
      
      // Scroll tracking
      if (config.autoTrackScroll) {
        console.log('[SOS] Auto-tracking scroll enabled');
        let scrollTimeout;
      ...
      if (config.autoTrackEngagement) {
        console.log('[SOS] Auto-tracking engagement enabled');
        engagedStart = Date.now();
      ...
      console.log('[SOS] Tracking started successfully');
    },
    ...
    _showConsentPopup: function() {
      try {
        ...
        console.error('[SOS] _showConsentPopup() error:', e);
      }
    }
  };
  
  console.log('[SOS] Debug version loaded, window.sos available');
